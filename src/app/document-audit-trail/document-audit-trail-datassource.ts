...
export class DocumentAuditTrialDataSource implements DataSource<DocumentAuditTrail> {

  private documentAuditTrailsSubject = new BehaviorSubject<DocumentAuditTrail[]>([]);
  ...
  loadDocumentAuditTrails(documentResource: DocumentResource) {
    ...
    this.documentAuditTrailService.getDocumentAuditTrials(documentResource).pipe(
      catchError(() => of([])),
